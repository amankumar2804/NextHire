...
export default function EditCareerNewsPage() {
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      try {
        ...
        // NOTE: this endpoint also increments view count, which is fine —
        // admin opening it to edit is a legitimate "view" too
        const response = await fetch(`${API_URL}/api/career-news/${articleId}`);
        ...
        setExistingThumbnail(article.thumbnailImage || "");
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      try {
        ...
        throw new Error("Update article API returned an invalid response");
      }
      ...
      router.push("/admin/career-resources/career-news");
      router.refresh();
    } catch (error) {
      console.error("Update Career News Error:", error);
    ...
    <main className="min-h-screen bg-slate-50">
      <section className="bg-gradient-to-br from-slate-950 via-indigo-950 to-purple-950 text-white">
        <div className="mx-auto max-w-3xl px-6 py-12">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="text-4xl font-black md:text-5xl">Edit Article</h1>
              <p className="mt-3 text-indigo-100">
                Update the headline, content or thumbnail.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-3xl px-6 py-12">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          {/* CONTENT */}
          <div className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm">
            <h2 className="mb-1 font-black text-slate-900">Full Article</h2>
