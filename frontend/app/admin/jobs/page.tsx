...
export default function JobsAdminListPage() {
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Jobs (Private / Government)</h1>
          <p className="mt-1 text-sm text-slate-500">
            Manage regular job listings shown on the Private and Government pages.
          </p>
        </div>

        <Link
          href="/admin/jobs/new"
          className="flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 font-bold text-white transition hover:bg-blue-700"
        >
          <Plus size={18} />
          Add New Job
        </Link>
      ...
      {!loading && jobs.length === 0 && (
        <div className="mt-16 rounded-2xl border border-slate-200 bg-white p-12 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Click "Add New Job" to post your first listing.
          </p>
        </div>
      )}
            ...
            <tbody>
              {jobs.map((job) => (
                <tr key={job._id} className="border-b border-slate-100 last:border-0">
                  ...
                  <td className="px-5 py-4 text-slate-600">{job.sector}</td>
