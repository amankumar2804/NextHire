...
export default function EditJobPage() {
  ...
  const [existingPdfPublicId, setExistingPdfPublicId] = useState("");
  ...
  useEffect(() => {
    ...
    const fetchJob = async () => {
      try {
        const res = await fetch(`${API_URL}/api/jobs/admin/id/${jobId}`, {
        ...
        setSector(job.sector || "Private");
        setLocation(job.location || "");
        setExperience(job.experience || "");
        setJobType(job.jobType || "Full-time");
        setSalary(job.salary || "");
        setLastDate(job.lastDate || "");
        setDescription(job.sector === "Government" ? "" : job.description || "");
        setArticle(job.sector === "Government" ? job.description || "" : "");
        setEligibility((job.eligibility || []).join("\n"));
        setSkills((job.skills || []).join(", "));
        setApplyUrl(job.applyUrl || "");
        setVerified(job.verified ?? true);
        ...
        setReferralContacts(
          (job.referralContacts || []).map((c: any) => ({
            name: c.name || "",
            role: c.role || "",
        ...
        setExistingPdfUrl(job.notificationPdf || "");
        setExistingPdfPublicId(job.notificationPdfPublicId || "");
        if (job.notificationPdf) {
          setPdfFileName(job.notificationPdf.split("/").pop() || "Notification.pdf");
        }
      } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let notificationPdf = existingPdfUrl;
      let notificationPdfPublicId = existingPdfPublicId;
      ...
      {!loading && (
        <form onSubmit={handleSubmit} className="mt-6 max-w-3xl">
          ...
          <div className="space-y-6 rounded-2xl border border-slate-200 bg-white p-6">
            ...
            {/* GOVERNMENT ONLY: NOTIFICATION PDF */}
            {isGovernment && (
              <div>
                <label className="mb-1.5 block text-sm font-bold text-slate-700">
                  Notification PDF
                ...
                {pdfFileName ? (
                  ...
                  >
                    ...
                    <span className="text-sm font-semibold">
                      Click to upload notification PDF
                    </span>
                  </button>
                )}
            ...
            <div>
              ...
              {isGovernment ? (
                <RichTextEditor
                  value={article}
                  onChange={setArticle}
                  placeholder="Write the full notification details here..."
                />
              ) : (
                <textarea
                  value={description}
            ...
            <div>
              <label className="mb-1.5 block text-sm font-bold text-slate-700">
                {isGovernment
                  ...
                  : "Apply URL (official company website)"}
              </label>
              <input
                type="text"
                value={applyUrl}
                    ...
                    >
                      ...
                      <div className="grid gap-3 pr-8 sm:grid-cols-2">
                        <input
                          ...
                          value={contact.name}
                          onChange={(e) =>
                            updateReferralContact(index, "name", e.target.value)
                          }
                          placeholder="Name"
                        ...
                        <input
                          ...
                          value={contact.role}
                          onChange={(e) =>
                            updateReferralContact(index, "role", e.target.value)
                          }
                          placeholder="Role"
                          className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-blue-400"
                        />
