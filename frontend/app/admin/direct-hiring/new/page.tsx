...
export default function NewDirectHiringJobPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Upload company image first (if one was selected)
      let companyImage = "";
      ...
      // 2. Create the job with the uploaded image URL
      const payload = {
        title: title.trim(),
        slug: slug.trim(),
  ...
  return (
    <div className="p-6">
      ...
      <h1 className="mt-4 text-2xl font-bold text-slate-900">Add New Direct Hiring Job</h1>
      <p className="mt-1 text-sm text-slate-500">
        This job will appear on the public Direct Hiring page once created.
      </p>
      ...
      <form onSubmit={handleSubmit} className="mt-6 max-w-3xl">
        ...
        <div className="space-y-6 rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <div>
            <label className="mb-1.5 block text-sm font-bold text-slate-700">
              Slug (optional — auto-generated from title + company if left blank)
            </label>
            <input
              type="text"
              value={slug}
        ...
        >
          ...
          {uploadingImage ? "Uploading image..." : "Create Job"}
        </button>
      </form>
    </div>
  );
